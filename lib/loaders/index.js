...
const detectLoaders = () => {
  ...
  const sqip = isModuleInstalled('@mole-inc/sqip-loader') ? '@mole-inc/sqip-loader' : false
  ...
  if (isModuleInstalled('@mole-inc/imagemin-mozjpeg')) {
    jpeg = '@mole-inc/imagemin-mozjpeg'
  } else if (isModuleInstalled('@mole-inc/imagemin-jpegoptim')) {
    jpeg = '@mole-inc/imagemin-jpegoptim'
  }
...
const getNumOptimizationLoadersInstalled = loaders => Object.values(loaders)
  .filter(loader => loader && (
    ...
    loader.startsWith('@mole-inc/sqip-') ||
